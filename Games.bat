...
echo -------------------------------------------------
echo #############################
echo WELCOME TO COOL-BAT-GAMES
echo #############################
echo version 1.2 (type update to update)
echo Created by luke miller
echo.
echo Have Fun!
echo.
echo -------------------------------------------------
...
echo.
echo 1 ---- Tetris
echo 2 ---- Risky Guesses
echo 3 ---- Tic Tac Toe
echo 4 ---- War Game
echo 5 ---- RPG
set /p option=
cls
if %option%==1 GOTO TETRIS
if %option%==2 GOTO GUESS
if %option%==3 GOTO TICTAC
if %option%==4 GOTO WAR12
if %option%==5 GOTO RPG12
if %option%==update ECHO update at https://github.com/sleepercreeper1/BatchGames
...
for /L %%i in (1,1,%lines%) do set "F%%i=  ณ%spc%ณ"
set /A top=lines+1
set "F%top%=  ฺ" & set "F0=  ภ"
for /L %%i in (1,1,%cols%) do set "F%top%=!F%top%!ฤ" & set "F0=!F0!ฤ"
set "F%top%=!F%top%!ฟ" & set "F0=%F0%ู"
...
%F16x8%           if %%i geq 1 if %%i leq %lines% echo(!F%%i!
               ...
               for /L %%i in (!i!,1,%lines%) do set "F%%i=  ณ%spc%ณ"
               call :Delay 95
               cls
               for /L %%i in (%top%,-1,-3) do (
                  echo(!F%%i!
